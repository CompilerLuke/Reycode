...
namespace reycode {
    void fpv_update(FPV& camera, const Input_State& input, real dt) {
        ...
        camera.capture_cursor = input_mouse_button_down(input, MOUSE_BUTTON_RIGHT);
        ...
        if (input_key_down(input, KEY_LEFT_SHIFT)) move_speed *= 2;

        if (input_key_down(input, KEY_BIND_UP)) camera.view_pos += move_speed * dt * forward;
        if (input_key_down(input, KEY_BIND_DOWN)) camera.view_pos += move_speed * dt * -forward;
        if (input_key_down(input, KEY_BIND_RIGHT)) camera.view_pos += move_speed * dt * right;
        if (input_key_down(input, KEY_BIND_LEFT)) camera.view_pos += move_speed * dt * -right;
    }
